...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    ...
                                    <h5 class="font-weight-bolder mb-0">
                                        {{ $accounts->where('status', 'Live')->count() }}
                                    </h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    ...
                                    <h5 class="font-weight-bolder mb-0">
                                        {{ $accounts->where('status', 'Die')->count() }}
                                    </h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
                ...
                <div class="card">
                    <div class="card-body p-3">
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    <p class="text-sm mb-0 text-capitalize font-weight-bold">Comeback</p>
                                    <h5 class="font-weight-bolder mb-0">
                                        {{ $accounts->where('status', 'Comeback')->count() }}
                                    </h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
                                <div class="icon icon-shape bg-gradient-primary shadow text-center border-radius-md">
                                    <i class="fa fa-arrow-left text-lg opacity-10" aria-hidden="true"></i>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="container-fluid pt-4 px-4">
        <form action="{{ route('customer.account.export') }}" method="get">
            <div class="row">
        ...
        <table id="myTable" class="stripe row-border order-column" width="100%">
            ...
            @foreach($accounts as $account)
                <tr>
                    ...
                    @endphp
                    @foreach($dates as $date)
                        @if($account->accountReports->where('date', $date->format('Y-m-d'))->first() && $account->accountReports->where('date', $date->format('Y-m-d'))->first()->spent != 0)
                            @php
                                $totalMonth+=$account->accountReports->where('date', $date->format('Y-m-d'))->first()->spent;
                            @endphp
                            <td style="color: #f8b600; font-weight: bold"> {{ sprintf("%.2f", $account->accountReports->where('date', $date->format('Y-m-d'))->first()->spent) }} </td> @else <td>0</td> @endif
                    @endforeach
